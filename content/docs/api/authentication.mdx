...
Better Auth's reset is keyed by email, but staff provisioned by an admin sign in with a
**username** (and may only have a synthetic address). This helper resolves a username to its
account and hands off to the normal reset flow.

| Method | Path | Notes |
| --- | --- | --- |
| `POST` | `/api/auth-helpers/reset-by-username` | Body `{ "username": "...", "redirectTo": "..." }` |

It always responds `{ "ok": true }` — it never reveals whether the username exists, and never
returns the resolved email. If an [email provider](/docs/guides/email-setup) is configured a
reset link is sent; otherwise the clinic admins are notified to set a new password.
